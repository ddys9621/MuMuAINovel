...
_SHOWOFF = BridgeTemplate(
    ...
    methodology=(
        "每个桥段 4 章，结构固定：\n"
        "- **C1 代入+信息差**（5:5）：上半日常代入，下半亮出对方困境，读者知道主角能解决而对方不知道\n"
...
_MYSTERY = BridgeTemplate(
    ...
    methodology=(
        "每个桥段 4 章，结构固定：\n"
        "- **C1 日常+异象疑点**（5:5）：上半日常代入，下半抛出一个反常细节或新案情，读者先于角色察觉不对\n"
...
_ROMANCE = BridgeTemplate(
    ...
    methodology=(
        "每个桥段 4 章，结构固定：\n"
        "- **C1 日常+情感缺口**（5:5）：上半日常代入，下半暴露一个未被满足的情感需要或误解的种子\n"
    ...
    card_hints={
        "c1_intro": "C1 日常代入素材 + 下半情感缺口/误解种子",
...
_INFINITE = BridgeTemplate(
    ...
    methodology=(
        "每个桥段 4 章，结构固定：\n"
        "- **C1 入局+规则展示**（5:5）：上半日常/结算代入，下半进入新局并亮出表面规则\n"
